...
export default function AddLand() {
  ...
  const { addLand, currentUser } = useAppStore()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitted(true)
      setTimeout(() => navigate(`${basePath}/lands`), 1500)
    } catch (err) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-sage-50" dir="rtl">
        <div className="text-center px-6">
          <div className="w-24 h-24 bg-green-500 rounded-full flex items-center justify-center mx-auto mb-4 shadow-xl">
            <Check size={48} className="text-white" />
          </div>
          <h2 className="text-2xl font-black text-green-700">تم نشر إعلان الأرض!</h2>
          <p className="text-gray-500 mt-2">سيظهر إعلانك قريباً</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-sage-50 pb-24" dir="rtl">
      ...
      >
        ...
        <h1 className="text-white text-2xl font-black">🌍 إضافة أرض فلاحية</h1>
        <p className="text-earth-200 text-sm mt-1">انشر إعلانك للمهتمين بشراء أو استئجار الأراضي</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="px-4 py-5 space-y-5">
        ...
        <button
          type="submit"
          className="w-full bg-green-500 text-white rounded-2xl py-4 font-black text-xl shadow-lg hover:bg-earth-600 active:scale-95 transition-all"
        >
          نشر إعلان الأرض 🌍
        </button>
      </form>
