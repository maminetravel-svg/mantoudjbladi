...
import { ArrowRight, Check, Camera, Plus, Trash2, Video, Image as ImageIcon, MapPin } from 'lucide-react'
...
export default function AddEquipment() {
  ...
  const { addEquipment, currentUser, equipmentTypeImages } = useAppStore()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitted(true)
      setTimeout(() => navigate(`${basePath}/equipment`), 1500)
    } catch (err) {
      alert('حدث خطأ أثناء إضافة الإعلان، تحقق من اتصالك بالإنترنت')
    }
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-sage-50" dir="rtl">
        <div className="text-center px-6">
          ...
          <h2 className="text-2xl font-black text-primary-700">تم نشر الإعلان!</h2>
          <p className="text-gray-500 mt-2">سيظهر إعلانك للجميع قريباً</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-sage-50 pb-24" dir="rtl">
      ...
      >
        ...
        <h1 className="text-white text-2xl font-black">🔧 إضافة معدة / أداة</h1>
        <p className="text-sage-300 text-sm mt-1">انشر إعلانك لآلاف الفلاحين</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="px-4 py-5 space-y-5">
        ...
        {/* GPS Location */}
        <div>
          <label className="block text-gray-700 font-black mb-1.5 text-base">الموقع على الخريطة (اختياري)</label>
          {gpsLat ? (
            <div className="flex items-center gap-2 bg-green-50 border-2 border-green-300 rounded-xl px-4 py-3">
              <MapPin size={16} className="text-green-600" />
              <span className="text-green-700 font-bold text-sm flex-1">{gpsLat.toFixed(4)}°N, {gpsLng!.toFixed(4)}°E</span>
              <button type="button" onClick={() => setShowMapPicker(true)} className="text-xs text-primary-600 font-bold underline">تغيير</button>
              <button type="button" onClick={() => { setGpsLat(null); setGpsLng(null) }} className="text-xs text-red-500 font-bold">حذف</button>
            </div>
          ) : (
            <button
              type="button"
              onClick={() => setShowMapPicker(true)}
              className="w-full border-2 border-dashed border-gray-300 rounded-xl py-4 flex items-center justify-center gap-2 text-gray-500 hover:border-primary-400 hover:text-primary-600 transition-colors"
            >
              <MapPin size={18} />
              <span className="font-bold text-sm">تحديد الموقع على الخريطة</span>
            </button>
          )}
        </div>

        {showMapPicker && (
          <React.Suspense fallback={<div className="h-64 flex items-center justify-center"><div className="w-8 h-8 border-4 border-primary-500 border-t-transparent rounded-full animate-spin" /></div>}>
            <MapPicker
              lat={gpsLat}
              lng={gpsLng}
              onChange={(lat: number, lng: number) => { setGpsLat(lat); setGpsLng(lng); setShowMapPicker(false) }}
